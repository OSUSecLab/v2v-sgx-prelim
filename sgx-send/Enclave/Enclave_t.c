...
static sgx_status_t SGX_CDECL sgx_prepare_broadcast(void* pms)
{
	...
	ms->ms_retval = prepare_broadcast(_in_encMessageOut, _tmp_lenOut);
...
static sgx_status_t SGX_CDECL sgx_receive_broadcast(void* pms)
{
	...
	ms->ms_retval = receive_broadcast(_in_sealed_data, _tmp_sealed_size, _in_rcvdEncMessageIn, _tmp_rcvdEncMessageInLen, _in_resultOut, _tmp_resultOutLen);
	if (_in_resultOut) {
