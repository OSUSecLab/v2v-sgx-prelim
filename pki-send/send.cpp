...
int main() {
    ...
    socket.connect( tcp::endpoint( boost::asio::ip::address::from_string("192.168.1.23"), 1234 ) );
...
// 
  ...
  const char cert_filestr[] = "cert_v1_ca_signed.pem";
             EVP_PKEY *pkey = NULL;
  BIO              *certbio = NULL;
  BIO               *outbio = NULL;
  X509                *certV1 = NULL;
  ...
  if (! (certV1 = PEM_read_bio_X509(certbio, NULL, 0, NULL))) {
    BIO_printf(outbio, "Error loading cert into memory\n");
    exit(-1);
  }

  string certV1_pemStr = cert_to_pem_string(certV1);
  // cout<< certV1_pemStr << endl;
...
// 
 ...
 // -------------------------------------------- Compute  ECDH ----------------------
  ...
  OID CURVE = secp256r1();
    ...
    ECDH < ECP >::Domain ecA( CURVE );


    SecByteBlock privA(ecA.PrivateKeyLength()), pubA(ecA.PublicKeyLength());
    ecA.GenerateKeyPair(rng, privA, pubA);
...
// -------------------------------------------- Socket Send ----------------------
   ...
   msg_send.nonce = nonceA; 
   msg_send.ec_agree_len = (int)(ecA.AgreedValueLength()); 
   msg_send.cert_pem_str = certV1_pemStr; 
   

   // cout<< "pubA.SizeInBytes(): " << pubA.SizeInBytes() << endl;
   std::copy(pubA.BytePtr(), pubA.BytePtr() + pubA.SizeInBytes(), msg_send.ec_pub); 
   

    // Integer pubA_int;
    // pubA_int.Decode(pubA.BytePtr(), pubA.SizeInBytes());
    // cout << "Send pubA_int :" << std::hex << pubA_int << endl;
...
// -------------------------------------------- Msg 2 Receive ----------------------
   ...
   // get binary content
    ...
    // std::cout << "Rcv nonceB: " << msg_rcv.nonce << std::endl;
...
//---------------------------------------------------------------------- Derive EDDH Keys ------------- 
  ...
  SecByteBlock sharedA(ecA.AgreedValueLength());
  
  if(msg_rcv.ec_agree_len != ecA.AgreedValueLength())
    throw runtime_error("Shared secret size mismatch");

    const bool rtn2 = ecA.Agree(sharedA, privA, msg_rcv.ec_pub);
    Integer a;
    a.Decode(sharedA.BytePtr(), sharedA.SizeInBytes());
    cout << " shared Key (A): " << std::hex << a << endl;

//---------------------------------------------------------------------- Key Derive Time ------------- 

cout << "keyDerive: "; print_time();

//---------------------------------------------------------------------- Send BSM msg 3 with ECDH shared key------------- 
    ...
    string key = string((char *)sharedA.BytePtr(), sharedA.SizeInBytes());
    
    // cout << "string encryption key :" << key << endl;

    //text to encrypt
    string plaintext= "Congestion at Olentengy";
    cout << plaintext << endl;
    ...
    vector<unsigned char> ciphertext = aes_128_gcm_encrypt(plaintext, key); 
      ...
      boost::asio::streambuf write_buffer;
      std::ostream output(&write_buffer);
      output << makeFixedLength(ciphertext.size(), HEADERSIZE);  
      for (unsigned char c: ciphertext) {
          output << c;
      }

      // std::cout << "Writing: " << make_string(write_buffer) << std::endl;
      auto bytes_transferred = boost::asio::write(socket, write_buffer);


//     
//---------------------------------------------------------------------- Release crypto pointer ------------- 
